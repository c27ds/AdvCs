...
class Strategy(bt.Strategy):
    def __init__(self):
        ...
        self.bbdiff = (self.bbands.lines.top - self.bbands.lines.bot)*100/self.data.close
        self.b = 0
        self.s = 0
        self.h = 0
    ...
    def next(self):
        self.debug(f'Close: {self.data.close[0]:.2f}, RSI: {self.rsi[0]:.2f}, MACD: {self.macd[0]:.2f}, ADX: {self.adx[0]:.2f}, BBDIFF: {self.bbdiff[0]:.2f}')
        # if self.position:
        #     if self.data.close[0] < self.stop_price:
        #         self.debug(f'Stop Loss at {self.data.close[0]:.2f}, Selling Position')
        #         self.close()
        ...
        if self.rsi[0] > 70 and self.macd_signal[0] > 0.1 and self.adx[0] > 27.5 and (self.bbdiff[0] < 5 and self.bbdiff[0] > 2):
            ...
            self.buy()
            self.stop_price = self.data.close[0] * 0.98
            
        elif self.rsi[0] < 30 and self.macd_signal[0] < -0.1 and self.adx[0] > 27.5 and self.position and (self.bbdiff[0] < 5 and self.bbdiff[0] > 2):
            self.s += 1
            self.debug(f'Sell Signal # {self.s}')
            self.sell()
        
        elif self.rsi[0] < 30 and self.macd_signal[0] < -0.1 and self.adx[0] > 27.5 and not self.position and (self.bbdiff[0] < 5 and self.bbdiff[0] > 2):
            self.h += 1
            self.debug(f'Short # {self.h}')
            self.short()
